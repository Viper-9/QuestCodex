...
describe('사전 파일 en.json / ko.json', () => {
  it('키 집합이 같다', () => {
  ...
  it('자리표시자({n} 등) 집합이 언어 간 같다', () => {
    for (const k of Object.keys(en) as (keyof typeof en)[]) {
      expect(placeholders(ko[k]), k).toEqual(placeholders(en[k]))
    }
  })
})
...
describe('getT', () => {
  it('언어별 문자열', () => {
    expect(getT('ko')('menu.wiki')).toBe('위키')
    expect(getT('en')('menu.wiki')).toBe('Wiki')
  })
  it('자리표시자 치환', () => {
    expect(getT('ko')('filter.count', { n: 612 })).toBe('612개 · 레벨순')
