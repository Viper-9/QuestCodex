...
describe('formatObjective', () => {
  it('text + ×N', () => {
    expect(lineText(formatObjective({ conditionId: 'c', conditionType: 'HandoverItem', text: '창고 열쇠 인도', targetName: null, targetCount: 1 }, ko))).toBe('창고 열쇠 인도 ×1')
    expect(lineText(formatObjective({ conditionId: 'c', conditionType: 'Visit', text: '창고 위치 확인', targetName: null, targetCount: null }, ko))).toBe('창고 위치 확인')
  })
  it('text 비고 targetName 있으면 conditionType + 아이템명, 톤은 일반', () => {
    expect(formatObjective({ conditionId: 'c', conditionType: 'HandoverItem', text: '', targetName: '황금 아령', targetCount: 1 }, ko))
      .toEqual({ parts: [{ text: 'HandoverItem: 황금 아령 ×1' }], tone: 'normal' })
  })
  it('text 비고 targetName 도 없으면 conditionType 만', () => {
    expect(formatObjective({ conditionId: 'c', conditionType: 'CounterCreator', text: '', targetName: null, targetCount: null }, ko)).toEqual({ parts: [{ text: 'CounterCreator' }], tone: 'normal' })
  })
})
